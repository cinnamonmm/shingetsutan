...
?><style>.bc-progress{width:100%;height:1pc;margin-bottom:1pc;background:#3798d9;background-image:linear-gradient(-45deg,hsla(0,0%,100%,.125) 25%,transparent 25%,transparent 50%,hsla(0,0%,100%,.125) 50%,hsla(0,0%,100%,.125) 75%,transparent 75%,transparent);background-size:34px 34px;transition:none 0 ease 0;animation:a 2s linear infinite}@keyframes a{to{background-position:34px 34px}}</style><?php
		...
		if( !isset( $luxe['jquery_load'] ) || ( isset( $luxe['jquery_load'] ) && $luxe['jquery_load'] === 'none' ) ) {
			...
			foreach( $luxe['bc_url'] as $i => $val ) {
				...
				if( isset( $caches[1] ) ) {
?><script>try{(function(){document.getElementById('bc_<?php echo $url_md5; ?>').outerHTML='<?php echo $caches[1]; ?>';})();}catch(e){console.error('bc_<?php echo $url_md5; ?>.error: '+e.message);}</script><?php
				}
...
?><script>try{var bcjck=function(c){if(window.jQuery){c(jQuery);}else{window.setTimeout(function(){bcjck(c);},100);}};bcjck(function($){var s=jQuery.ajax({type:"POST",url:"<?php echo $ajaxurl; ?>",data:{action:"thk_blogcard_cache",bc_url:'<?php echo $val; ?>',bc_md5:'<?php echo $luxe['bc_md5'][$i]; ?>',bc_lnk:'<?php echo $luxe['bc_lnk'][$i]; ?>',blogcard_cache_nonce:'<?php echo $blogcard_cache_nonce; ?>'},dataType:"json",async:true,cache:false,timeout: 300000}).then(function(r){try{(function(){document.getElementById("bc_"+r[0]).outerHTML=r[1];})();}catch(e){console.error("bc_"+r[0]+".error: "+e.message);}console.log("Create blogcard cache. Success: bc_<?php echo $luxe['bc_md5'][$i]; ?>");},function(){console.log("Create blogcard cache. Failed: bc_<?php echo $luxe['bc_md5'][$i]; ?>");s.abort();});});}catch(e){}</script><?php
				unset( $luxe['bc_url'][$i], $luxe['bc_md5'][$i], $luxe['bc_lnk'][$i] );
			}
		}
	}
